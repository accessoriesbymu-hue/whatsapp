...
async function setupLocalFileDB() {
    try {
            ...
            if (fs.existsSync(sessionPath)) {
                ...
                if (!ud.client) {
                    try {
                        await createClient(username);
                        ud.client.initialize().catch(err => {
                            console.error(`[AUTO-INIT][${username}] Initialization error:`, err.message);
                            ud.client = null;
                            ud.isReady = false;
                            ud.isInitializing = false;
                        });
                    } catch (clientErr) {
                        console.error(`[AUTO-INIT][${username}] Client creation error:`, clientErr.message);
                        ud.client = null;
                        ud.isReady = false;
                        ud.isInitializing = false;
                    }
